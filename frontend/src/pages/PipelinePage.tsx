...
export function PipelinePage() {
  ...
  // Single source of truth for the selected invoice's pipeline view.
  // Lifted up here so the sidebar badge and the main visualizer agree
  // on whether the catch-up animation is still in progress -- without
  // this, the sidebar would read "Completed" off the server status
  // while the main view still showed the Matcher node spinning.
  const pipeline = useLivePipeline(selectedId ?? undefined)
          ...
          <Card className="h-fit lg:sticky lg:top-24">
            <CardContent className="p-3 space-y-1">
              ...
              {invoices.slice(0, 12).map((inv) => {
                ...
                // For the selected invoice, prefer the visualizer's
                // displayed status so the sidebar doesn't read
                // "Completed" while the main view still spins through
                // matcher / anomaly / resolution.
                const badgeStatus =
                  isSelected && pipeline.isProcessing && pipeline.displayedStatus
                    ? pipeline.displayedStatus
